...
Create `/etc/nginx/sites-available/annotations-server-ssl` and add the following:
...
server {
  ...
  location / {
    proxy_pass http://ip-172-31-2-81.us-east-2.compute.internal:8081;
    proxy_http_version 1.1;
    proxy_set_header Upgrade $http_upgrade;
    proxy_set_header Connection 'upgrade';
    proxy_set_header Host $host;
...
The security policy for the annotations service host should open those ports used for public access (e.g., 80, 8000, 8443) depending on what is needed and whether production or development mode is used.
...
sudo ln -s /etc/nginx/sites-available/annotations-server-ssl /etc/nginx/sites-enabled/annotations-server-ssl
```
